...
case $action in
    download-data)
        ...
        echo "CHANGE DATA FILES TO IMPORT !!"
        echo "CHANGING allCountries.txt"
        sed -i 's/\\//g' "$zip_codes_dir/allCountries.txt"
        echo "CHANGING iso-languagecodes.txt"
        sed -i '1d' "$data_dir/iso-languagecodes.txt"
        echo "CHANGING timeZones.txt"
        sed -i '1d' "$data_dir/timeZones.txt"
        echo "CHANGING countryInfo.txt"
...
case "$action" in
    ...
    truncate-db)
        echo "Truncating $dbname database"
